function G = bst_eeg_sph_cjb(Rq, Re, center, R, sigma)
...
% center MUST be origin!
% Re = Re - center(:)';
% Rq = Rq - center(:)';
% Compute Berg parameters
[mu_berg, lam_berg] = bst_berg_cjb(R, sigma);
...
%         Rq(nx,:) = Re_mag.^2 * bst_bsxfun(@rdivide, Rq(nx,:), sum(Rq(nx,:).^2,2));
        Rq(nx,:) = Re_mag.^2 * Rq(nx,:) ./ sum(Rq(nx,:).^2,2);
    end
...
%     c2 = bst_bsxfun(@times, (2./d_mag.^3) + (d_mag+Re_mag) ./ (Re_mag.*F_scalar), const);
    cc1 = (2*( (mu.*Re_dot_Rq - Rq1_mag_sq) ./ d_mag.^3) + 1./d_mag - 1./Re_mag);
    cc2 = (2./d_mag.^3) + (d_mag+Re_mag) ./ (Re_mag.*F_scalar);
    c1 = zeros(size(cc1)); c2 = zeros(size(cc2));
    for jj = 1:size(Re,1)
        c1(:,jj) = cc1(:,jj) .* const;                 %(PxM)
        c2(:,jj) = cc2(:,jj) .* const;
    end
    %
